...
const CompanionCard = ({ id, name, topic, duration, color, subject }: CompanionCardProps) => {
    return (
        <article className='hover:shadow-md shadow-gray-500 flex flex-col rounded-2xl border border-black px-4 py-4 gap-3 w-full min-lg:max-w-[450px] justify-between bg-cyan-900 transition-all duration-300 ease-in-out' style={{ backgroundColor: color }}>
            ...
            <Link href={`/companions/${id}`} className='text-black font-bold text-md '>
                <Button className='bg-[#58A6FF] hover:bg-[#1F6FEB] w-full h-10 rounded-lg '>Launch lesson</Button>
            </Link>
        </article>
    )
}
